...
PUBLIC_KEY = ''
PRIVATE_KEY = ''
API_URL = 'http://' + API_DOMAIN + API_PATH
...
def doRequest(method, params):
	...
	oauth_request = oauth.OAuthRequest.from_consumer_and_token(consumer, token=token, http_method='GET', http_url=API_URL + "/json/" + method, parameters=params)
...
def requestToken():
	global config
	consumer = oauth.OAuthConsumer(PUBLIC_KEY, PRIVATE_KEY)
	request = oauth.OAuthRequest.from_consumer_and_token(consumer, http_url=API_URL + '/oauth/requestToken')
	request.sign_request(oauth.OAuthSignatureMethod_HMAC_SHA1(), consumer, None)
	conn = httplib.HTTPConnection(API_DOMAIN + ":" + API_PORT)
	conn.request(request.http_method, API_PATH + '/oauth/requestToken', headers=request.to_header())

	resp = conn.getresponse().read()
	token = oauth.OAuthToken.from_string(resp)
	print 'Open the following url in your webbrowser:\n' + API_URL + '/oauth/authorize?oauth_token=%s\n' % token.key
...
def getAccessToken():
	...
	request = oauth.OAuthRequest.from_consumer_and_token(consumer, token=token, http_method='GET', http_url=API_URL + '/oauth/accessToken')
